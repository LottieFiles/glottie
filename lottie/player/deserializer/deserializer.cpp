	...
	//switch (theScope->scope) {
		...
		case assets:
			if (currentReadKey == "layers") {
				EM_ASM({console.log('found assets_layers');});
				addScope(assets_layers);
			}
			break;
		case assets_layers:
			if (currentReadKey == "shapes") {
				EM_ASM({console.log('found assets_layers_shapes');});
		...
		case assets_layers_shapes:
			if (currentReadKey == "ks") {
				EM_ASM({console.log('found assets_layers_shapes_ks');});
		...
		case assets_layers_shapes_ks:
			if (currentReadKey == "k") {
				EM_ASM({console.log('found assets_layers_shapes_ks_k');});
				addScope(assets_layers_shapes_ks_k);
			}
			break;
		case assets_layers_shapes_ks_k:
			if (currentReadKey == "e") {
				EM_ASM({console.log('found assets_layers_shapes_ks_k_e');});
				addScope(assets_layers_shapes_ks_k_e);
			}
			if (currentReadKey == "s") {
				EM_ASM({console.log('found assets_layers_shapes_ks_k_s');});
...
bool isReadingDone() {
			...
			if (theState->stateNow == KVReading || theState->stateNow == KVReadOpen) {
				...
				if (kvState == Value) {
					EM_ASM({console.log("reading 100.2.0");});
					...
					EM_ASM({console.log("reading 100.3.1");});
					currentReadKey = currentValue;
					EM_ASM({console.log("reading 100.3.2");});
					readingDone();
					/*
					if (currentReadKey == "assets") {
						EM_ASM({console.log('found assets');});
...
int checkCharacter(char& currentChar) {
	...
	switch (currentChar) {
		case '{':
			...
			readingArray = false;
			if (theScope->prev != NULL) {
				EM_ASM_({console.log("-->added this " + $0 + " from " + $1 + " -- " + $2);}, (int)theScope->scope, (int)theScope->prev->scope, (int)lastScopeBeforeObject());
			}
